...
class DynamicFormWidget extends \yii\base\Widget
{
    ...
    public function registerAssets()
    {
        ...
        $js = '$("#' . $this->formId . '").yiiDynamicForm(' . $this->_hashVar . ');' . "\n";
        $view->registerJs($js, $view::POS_READY);

        // add a click handler for the clone button
        $js = '$("#' . $this->formId . '").on("click", "' . $this->insertButton . '", function(e) {' . "\n";
        $js .= "    e.preventDefault();\n";
        $js .= '    var container = $(this).closest("div.' . $this->widgetContainer . '");' . "\n";
        $js .= '    var formId = container.data("dynamicform");' . "\n";
        $js .= '    container.triggerHandler("beforeInsert", [$(this)]);' . "\n";
        $js .= '    container.yiiDynamicForm("addItem", eval(formId), e, ' . "$(this));\n";
        $js .= "});\n";
        $view->registerJs($js, $view::POS_READY, $this->formId . '.' . $this->insertButton);

        // add a click handler for the remove button
        $js = '$("#' . $this->formId . '").on("click", "' . $this->deleteButton . '", function(e) {' . "\n";
        $js .= "    e.preventDefault();\n";
        $js .= '    var container = $(this).closest("div.' . $this->widgetContainer . '");' . "\n";
        $js .= '    var formId = container.data("dynamicform");' . "\n";
        $js .= '    container.yiiDynamicForm("deleteItem", eval(formId)' . ", e, $(this));\n";
        $js .= "});\n";
        $view->registerJs($js, $view::POS_READY, $this->formId . '.' . $this->deleteButton);
    }
